...
import os, glob
# Search for CSVs relative to the script location (handles different working directories)
base_dir = os.path.dirname(__file__) or os.getcwd()
csv_paths = glob.glob(os.path.join(base_dir, '*.csv'))
# If not found in the same folder, search recursively inside the project folder
if not csv_paths:
    csv_paths = glob.glob(os.path.join(base_dir, '**', '*.csv'), recursive=True)
csv_files = [os.path.basename(p) for p in csv_paths]
csv_map = {os.path.basename(p): p for p in csv_paths}
if not csv_files:
    st.error('No CSV files found in project folder. Please add the datasets (e.g. diet_recommendations_dataset.csv, INDB.csv) to the project directory.')
    st.stop()
user_choice = st.sidebar.selectbox('Select User Profile Dataset (diet_recommendations_dataset.csv)', csv_files, index=csv_files.index('diet_recommendations_dataset.csv') if 'diet_recommendations_dataset.csv' in csv_files else 0)
food_choice = st.sidebar.selectbox('Select Food/Nutrition Dataset (INDB.csv)', csv_files, index=csv_files.index('INDB.csv') if 'INDB.csv' in csv_files else 0)
# Map selected names back to full paths
user_file = csv_map[user_choice]
food_file = csv_map[food_choice]


# Load user profile dataset (patient profiles)
st.write('Debug: discovered CSV files:', csv_files)
st.write('Debug: selected user_file path:', user_file)
st.write('Debug: selected food_file path:', food_file)
# Ensure selections are valid
if not user_file:
    st.error('No user CSV selected or file not found. Please add datasets (e.g. diet_recommendations_dataset.csv) to the project directory and refresh.')
    st.stop()
if not food_file:
    st.error('No food CSV selected or file not found. Please add datasets (e.g. INDB.csv) to the project directory and refresh.')
    st.stop()
